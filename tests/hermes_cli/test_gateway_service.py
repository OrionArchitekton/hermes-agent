...
class TestSystemdServiceRefresh:
    ...
    def test_refresh_keeps_current_dropin_when_project_root_is_site_packages(
        ...
        current_root = tmp_path / "hermes-agent-v31"
        package_root = current_root / "venv" / "lib" / "python3.13" / "site-packages"
        (current_root / "venv" / "bin").mkdir(parents=True)
        ...
        dropin_dir = tmp_path / "hermes-gateway.service.d"
        dropin_dir.mkdir()
        current_dropin = dropin_dir / "90-doppler-wrap.conf"
        current_dropin.write_text(
            "[Service]\n"
            "ExecStart=\n"
            f"ExecStart=/usr/bin/doppler run -- {current_root}/venv/bin/python "
        ...
        monkeypatch.setattr(gateway_cli, "PROJECT_ROOT", package_root)
        monkeypatch.setattr(
            gateway_cli, "get_python_path",
            lambda: str(current_root / "venv" / "bin" / "python"),
        )
        monkeypatch.setattr(
            gateway_cli, "get_systemd_unit_path", lambda system=False: unit_path
        )
    ...
    def test_refresh_disables_stale_profile_gateway_execstart_dropin(
        self, tmp_path, monkeypatch
    ):
        current_root = tmp_path / "hermes-agent-v31"
        retired_root = tmp_path / ".hermes" / "hermes-agent"
        ...
        unit_text = (
            "[Service]\n"
            "ExecStart=/current/python -m hermes_cli.main --profile coder gateway run\n"
        )
        unit_path.write_text(unit_text, encoding="utf-8")
        dropin_dir = tmp_path / "hermes-gateway-coder.service.d"
        dropin_dir.mkdir()
        stale_dropin = dropin_dir / "90-doppler-wrap.conf"
        stale_dropin.write_text(
            "[Service]\n"
            "ExecStart=\n"
            f"ExecStart=/usr/bin/doppler run -- {retired_root}/venv/bin/python "
            "-m hermes_cli.main --profile coder gateway run --replace\n",
            encoding="utf-8",
        )
        ...
        assert gateway_cli.refresh_systemd_unit_if_needed(system=False) is True
        assert not stale_dropin.exists()
        assert len(list(dropin_dir.glob("90-doppler-wrap.conf.disabled-*"))) == 1
...
class TestGeneratedSystemdUnits:
    ...
    def test_user_unit_marks_planned_stop_without_recursive_execstop_and_uses_extended_stop_timeout(self, monkeypatch):
        monkeypatch.setattr(
            gateway_cli,
    ...
    def test_system_unit_marks_planned_stop_without_recursive_execstop_and_uses_extended_stop_timeout(self, monkeypatch):
        monkeypatch.setattr(
            gateway_cli,
            "_get_restart_drain_timeout",
            lambda: DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT,
